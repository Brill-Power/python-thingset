...
class ThingSetBinaryEncoder(object):
    ...
    def encode_exec(self, value_id: int, args: Union[Any, None]) -> bytes:
        ...
        return bytes([ThingSetRequest.EXEC] + list(cbor2.dumps(value_id)) + list(cbor2.dumps(p_args, canonical=True)))
    ...
    def encode_update(self, parent_id: int, value_id: int, value: Any) -> bytes:
        ...
        return bytes([ThingSetRequest.UPDATE] + list(cbor2.dumps(parent_id)) + list(cbor2.dumps({value_id:value}, canonical=True)))
